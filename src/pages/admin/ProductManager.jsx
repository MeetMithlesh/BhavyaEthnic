...
export default function ProductManager() {
  ...
  const categories = useMemo(
    () => [...new Set(products.map((product) => product.category).concat(["Kurtas", "Suit Sets", "Dresses", "Festive"]))],
    [products]
  );
  ...
  const submit = async (event) => {
    ...
    // Client-side guard for description
    ...
    try {
      ...
      const payload = {
        ...
        sizes: form.sizes.map((item) => ({ size: item.size, stock: Number(item.stock) || 0 })),
      };

      console.log("Submitting Product Payload:", payload); // Debug to inspect in browser console
      ...
      <form onSubmit={submit} className="h-fit rounded-md bg-white p-5 ring-1 ring-clay/20">
        ...
        <div className="mt-5 grid gap-3">
          ...
          <textarea
            ...
            rows="4"
            className="rounded-md border border-clay/30 px-3 py-2 outline-none focus:border-terracotta"
          />
          ...
          <textarea
            ...
            onChange={(event) => setForm({ ...form, replacementPolicy: event.target.value })}
            rows="2"
                ...
                <label key={item.size} className="text-xs font-semibold text-stone">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setForm((current) => ({
                        ...current,
                        sizes: current.sizes.map((size, i) =>
                          i === index ? { ...size, stock: event.target.value } : size
                        ),
                      }))
                    }
                    type="number"
                    min="0"
                    className="mt-1 h-10 w-full rounded-md border border-clay/30 px-2 text-sm outline-none"
                  />
                </label>
              ))}
            </div>
          ...
          <label className="grid cursor-pointer place-items-center rounded-md border border-dashed border-clay/50 p-5 text-center text-sm text-stone">
            ...
            <input
              ...
              accept="image/*"
              className="hidden"
              onChange={(event) => addImagePreviews(event.target.files)}
            />
          </label>
          ...
          {imageFiles.length > 0 && (
            <p className="text-xs font-semibold text-terracotta">
              {imageFiles.length} image{imageFiles.length > 1 ? "s" : ""} selected. They will upload when you save.
            </p>
          )}
      ...
      <div className="rounded-md bg-white p-5 ring-1 ring-clay/20">
        ...
        <div className="mt-5 grid gap-4">
          ...
          {products.map((product) => (
            <div
              key={product._id}
              className="grid gap-4 rounded-md border border-clay/20 p-3 sm:grid-cols-[80px_1fr_auto]"
            >
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {product.sizes?.map((size) => (
                    <span key={size.size} className="rounded-md bg-blush px-2 py-1 text-xs">
              ...
              <div className="flex gap-2 sm:flex-col">
                <button
                  ...
                  className="grid size-10 place-items-center rounded-md bg-blush text-ink"
                >
                  <Pencil size={17} />
                </button>
                <button
                  ...
                  className="grid size-10 place-items-center rounded-md bg-terracotta text-white"
                >
                  <Trash2 size={17} />
                </button>
              </div>
